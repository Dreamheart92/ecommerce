...
const imageStyle = 'w-full h-full object-cover rounded-sm cursor-pointer transition-opacity ease-in-out absolute duration-500 -z-10';

export default function Card({ item, size, mobileSize, isOnWishlist }) {
    ...
    if (isOnMobile) {
        return (
            <section
                ...
                className="relative w-full wishlist-container mb-14">
                <Link to={`/details/${item._id}`}>
                    <div
                        className={mobileSize}>
                        <img className="object-cover w-full h-full" src={item.images[0]} alt="" />
                    </div>
                    <div>
    ...
    return (
        <section
            id="wishlist-container"
            className="relative w-full h-full wishlist-container text-sm">
            <Link to={`/details/${item._id}`}>
                <div
...
export default function Card({ item, size, isOnWishlist }) {
    ...
    return (
        <section
            id="wishlist-container"
            className="relative w-full wishlist-container text-sm">
            <Link to={`/details/${item._id}`}>
                <div
                    className={size + ' relative overflow-hidden'}
                    onMouseEnter={handleMouseEnter}
