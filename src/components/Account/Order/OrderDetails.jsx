...
export default function OrderDetails() {
    ...
    return (
        <section className="w-[80%] text-sm">
            <h1 className="font-semibold text-xl mb-2">Order details</h1>

            <section className="flex gap-2 flex-wrap">
                <section className="w-full box-shadow flex justify-between p-4">
                    <h1>Date of order </h1>
                    <p>{date}</p>
                </section>

                <section className="flex w-full justify-between p-4  box-shadow">
                    <h1>Order</h1>
                    <p>#{order._id}</p>
                </section>
            </section>

            <section className="w-full p-4  box-shadow mt-4">
                ...
                {order.products.map(product => {
                    return (
                        <section
                            ...
                            <section className="p-4 text-stone-500 flex flex-col gap-1">
                                ...
                                <p>Price: {product.quantity} x ${product._id.price.toFixed(2)}</p>
...
export const loader = async ({ request, params }) => {
    ...
    const order = await fetch('http://192.168.0.189:3000/orders/' + orderId);
