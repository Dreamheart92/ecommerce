...
export default function Header() {
    ...
    const textColor = isScrolled || !isHomePage || isSearchShowed || isMegaMenuShown ? ' text-stone-900' : ' text-stone-100';

    let headerStyle = 'flex flex-col justify-between items-center w-screen fixed top-0 z-[100] py-4' + textColor;

    if (isScrolled || !isHomePage || isSearchShowed || isMegaMenuShown) {
        headerStyle += ' bg-white border'
    }

    return (
        <header className={headerStyle}>
            <section className="absolute top-12 left-0 bg-white shadow-xl w-full h-fit z-50" id="search-results"></section>
            <section className="flex items-center w-full h-full justify-around text-[.9em]">
                <Menu isMegaMenuShown={isMegaMenuShown} setIsMegaMenuShown={setIsMegaMenuShown} />
            ...
            <section id="dropdown" className="absolute w-full bottom-0"></section>
